		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				...
				<view style="display: flex;justify-content: space-between;" v-if="product.yonghuId == user.id">
					<text v-if="isAuth('ziliao','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('ziliao','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('ziliao','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`ziliao`, params);
			...
			async search() {
				...
				let res = await this.$api.list(`ziliao`, searchForm);
				// 如果是第一页数据置空
