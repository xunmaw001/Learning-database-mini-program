		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.ziliaoName}}
					</view>
				...
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'
					@tap="download(detail.ziliaoFile)">
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"#4477C5","textAlign":"left"}'>
						文件：</view>
					<view class="text" :style='{"color":"#4477C5","textAlign":"center","fontSize":"24rpx"}'>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('ziliao', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.ziliaoPhoto ? this.detail.ziliaoPhoto.split(",") : [];
				
				let d = await this.$api.page('guankanjilu', {
					page: 1,
					limit: 99,
					ziliaoId: this.id,
				...
				if(d.data.list.length<=0){
					...
					await this.$api.save('guankanjilu', data);
				}
				
			},
			// 收藏
			async getziliaoCollection() {
				let params = {
					page: 1,
					limit: 1,
					ziliaoId: this.id,
				...
				let res = await this.$api.list(`ziliaoCollection`, params);
			...
			async shoucang() {
				...
				let params = {
					page: 1,
					limit: 1,
					ziliaoId: _this.detail.id,
					yonghuId: _this.user.id,
					ziliaoCollectionTypes: '1'
				}
				let res = await _this.$api.list(`ziliaoCollection`, params);
				if (res.data.list.length == 1) {
					let ziliaoCollectionId = res.data.list[0].id;
					uni.showModal({
						title: '提示',
						content: '是否取消',
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('ziliaoCollection', JSON.stringify([
									ziliaoCollectionId]));
								_this.$utils.msg('取消成功');
								_this.getziliaoCollection();
							}
						}
					});
					return;
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('ziliaoCollection', {
								yonghuId: _this.user.id,
								ziliaoId: _this.detail.id,
								ziliaoCollectionTypes: 1
							});
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('ziliaoLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					ziliaoId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../ziliaoLiuyan/add-or-update?ziliaoId=${this.id}`)
			},
		}
	}
</script>
