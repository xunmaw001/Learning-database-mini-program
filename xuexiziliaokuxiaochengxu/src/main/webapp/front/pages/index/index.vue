	...
	<view class="uni-padding-wrap">
		...
		<view class="list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' @tap="onNewsDetailTap(item.id)"
				...
				<view class="listmr">
					<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#4477C5","textAlign":"left"}'
		...
		<view v-if="ziliaoList1.length>0" class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			:style='{"backgroundColor":"#07588D","borderColor":"#4477C5","color":"rgba(255, 255, 255, 1)","borderRadius":"200rpx","borderWidth":"4rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"dashed"}'>
			<view class="left" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				热门阅读资料展示
			</view>
			<view class="right-content" @tap="onPageTap('ziliao')" :style="{fontSize:'32rpx',color:'#FFF'}">
				查看更多
			</view>
		</view>
		<view v-if='3 == 3' class="uni-product-list" :class="1 == 1 ? 'active' : ''"
			:style='{"borderRadius":0,"backgroundColor":"rgba(178, 178, 178, 0)"}'>
			<view @tap="onDetailTap('ziliao',product.id)" :style='{"borderRadius":"40rpx","backgroundColor":"#4477C5"}'
				class="uni-product" v-for="(product,index) in ziliaoList1" :key="index">
				<view
					:style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 253, 253, 1)","textAlign":"center"}'
					class="uni-product-title">{{product.ziliaoName}}</view>
		...
		<view v-if="ziliaoList.length>0" class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			:style='{"backgroundColor":"#07588D","borderColor":"#4477C5","color":"rgba(255, 255, 255, 1)","borderRadius":"200rpx","borderWidth":"4rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"dashed"}'>
			<view class="left" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				阅读资料展示
			</view>
		...
		<view v-if='3 == 3' class="uni-product-list" :class="1 == 1 ? 'active' : ''"
			:style='{"borderRadius":0,"backgroundColor":"rgba(178, 178, 178, 0)"}'>
			<view @tap="onDetailTap('ziliao',product.id)" :style='{"borderRadius":"40rpx","backgroundColor":"#4477C5"}'
				class="uni-product" v-for="(product,index) in ziliaoList" :key="index">
				<view
					:style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 253, 253, 1)","textAlign":"center"}'
					class="uni-product-title">{{product.ziliaoName}}</view>
				<view class="image-view" :style='{"borderRadius":"8rpx","height":"200rpx"}'>
...
<script>
	...
	export default {
		...
		async onShow() {
			...
			let ziliao = await this.$api.list('ziliao', {
				page: 1,
				limit: 6,
				ziliaoYesnoTypes:2
			});
			this.ziliaoList = ziliao.data.list

			let ziliaoList1 = await this.$api.list('ziliao', {
				page: 1,
				limit: 6,
				ziliaoYesnoTypes:2,
				orderBy:"ziliao_click_number"
			});
			this.ziliaoList1 = ziliaoList1.data.list
