	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.ziliaoName" type="text" placeholder="阅读资料名称"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in ziliaoTypesList" :key="index"
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				:style='{"borderRadius":"50rpx","backgroundColor":"#4477C5"}' v-for="(product,index) in list"
				:key="index">
				...
				<view style="display: flex;justify-content: space-between;" v-if="product.yonghuId == user.id">
					<text v-if="isAuth('ziliaow','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('ziliaow','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('ziliaow','新增')" class="add-btn" @click="onAddTap()">新增</button>
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.ziliaoTypes = this.categoryName
				}

				let res = await this.$api.page(`ziliao`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('ziliao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				let res = await this.$api.page(`ziliao`, searchForm);
				// 如果是第一页数据置空
